...
def ej1_max_in_list(lista):
    #Creamos una variable para almacenar el maximo numero empezando por cero, despues vamos leyendo uno a uno los mensajes de la lista dada i vamos comparando uno a uno los valores de la lista con el valor inicial, si es mayor el indice del loop registrado en la lista, actualiza la variable que contiene el valor inicial con el indice mas grande, asi hasta que termina de leer y devolvernos el numero mas alto.
    val_max = 0
    for i in lista:
        if i > val_max:
            val_max = i
            continue
    print(val_max)
def ej2_mas_larga(listaStr):
    maxNum = int(0)
    palabra = [0]
    maxPalabra = ""
    for i in listaStr:
        palabra.append(len(i))
